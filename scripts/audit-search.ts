      ...
      for (const [idx, msg] of s.messages.entries()) {
        ...
        if ((msg.role === 'user' || msg.role === 'assistant') && (msg.content || msg.toolCalls?.length)) {
          ...
          insertFts.run(
            ...
            s.cwd || '',
            tagsStr,
            // TOMB-21 修复后：与 messages_cache.id 一致的复合 id
            msgId,
            s.id,
            s.platform,
            msg.role
...
function runViewAndFilterAudit(): void {
  ...
  // 9. raw_content join 检查（TOMB-21：message_id 必须与 messages_cache.id 一致）
  const db = cacheService.getDb()!
  ...
  if (joinCheck.missed !== 0) {
    throw new Error(`[join] 断言失败：${joinCheck.missed}/${joinCheck.total} 条 FTS 记录无法连接 messages_cache`)
  }

  // 10. 锚点检查：搜索结果 message_id 必须能在 messages_cache 中定位到对应消息（前端跳转的前提）
  const msgExists = db.prepare('SELECT 1 FROM messages_cache WHERE id = ? AND platform = ?')
  const anchorQuery = '检索'
  const flatRes = cacheService.search(anchorQuery, { groupBy: 'message', limit: 100 })
  const groupedRes = cacheService.search(anchorQuery, { groupBy: 'session', limit: 100 })
  let anchorChecked = 0
  let anchorFailed = 0
  const checkAnchor = (platform: string, messageId: string) => {
    anchorChecked++
    if (!msgExists.get(messageId, platform)) {
      anchorFailed++
      console.error(`[anchor] ❌ 无法定位 platform=${platform} message_id=${messageId}`)
    }
  }
  for (const r of flatRes.results) checkAnchor(r.platform, r.message_id)
  for (const g of groupedRes.groupedSessions || []) {
    for (const snip of g.snippets) checkAnchor(g.platform, snip.message_id)
  }
  console.log(`[anchor] 平铺视图 + 聚合视图锚点定位: ${anchorChecked - anchorFailed}/${anchorChecked} 通过`)
  if (anchorChecked === 0) throw new Error('[anchor] 断言失败：没有可检查的搜索结果')
  if (anchorFailed !== 0) throw new Error(`[anchor] 断言失败：${anchorFailed} 条结果无法定位到消息`)

  // 11. raw_content 检查：平铺视图搜索应返回非 NULL 的 raw_content
  const rawCheck = db.prepare(`
    SELECT substr(m.content, 1, 160) AS raw_content
    FROM fts_messages f
    LEFT JOIN messages_cache m ON m.id = f.message_id
    WHERE fts_messages MATCH ?
    LIMIT 5
  `).all('"检索"') as Array<{ raw_content: string | null }>
  const nullCount = rawCheck.filter(r => r.raw_content === null).length
  console.log(`[raw_content] 抽样 ${rawCheck.length} 条，NULL ${nullCount} 条`)
  if (rawCheck.length > 0 && nullCount !== 0) {
    throw new Error(`[raw_content] 断言失败：${nullCount}/${rawCheck.length} 条 raw_content 为 NULL`)
  }
}
  ...
  const tx = db.transaction(() => {
    for (let i = 0; i < sessionCount; i++) {
      ...
      for (let j = 0; j < msgsPerSession; j++) {
        ...
        insertFts.run(segmentText(segmenter, content), titleSeg, cwd, `${platform}_${sid}_${j}`, sid, platform, j % 2 ? 'assistant' : 'user')
      }
    }
  })
  const t0 = performance.now()
