  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80">
        <div class="text-xs text-zinc-500 dark:text-zinc-400">总沉淀资产</div>
        <div class="text-2xl font-bold font-mono mt-1 text-zinc-900 dark:text-zinc-100">{{ totalCount }}</div>
      </div>
      <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80">
        <div class="text-xs text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
          <UIcon name="i-lucide-shield-check" class="w-3.5 h-3.5" /> 架构决策 (ADR)
        </div>
        <div class="text-2xl font-bold font-mono mt-1 text-emerald-600 dark:text-emerald-400">{{ adrCount }}</div>
      </div>
      <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80">
        <div class="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1">
          <UIcon name="i-lucide-alert-triangle" class="w-3.5 h-3.5" /> 避坑锦囊 (Gotchas)
        </div>
        <div class="text-2xl font-bold font-mono mt-1 text-amber-600 dark:text-amber-400">{{ gotchaCount }}</div>
      </div>
      <div class="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80">
        <div class="text-xs text-blue-600 dark:text-blue-400 flex items-center gap-1">
          <UIcon name="i-lucide-box" class="w-3.5 h-3.5" /> 工程模板 (Patterns)
        </div>
        <div class="text-2xl font-bold font-mono mt-1 text-blue-600 dark:text-blue-400">{{ patternCount }}</div>
      </div>
    </div>
    ...
    <!-- Items Grid -->
    <div v-if="status === 'pending'" class="py-12 text-center text-zinc-400 text-sm">
      <UIcon name="i-lucide-loader-2" class="w-6 h-6 animate-spin mx-auto mb-2" />
      正在加载知识资产...
    </div>

    <div v-else-if="items.length === 0" class="py-16 text-center bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80">
      <UIcon name="i-lucide-folder-archive" class="w-12 h-12 text-zinc-300 dark:text-zinc-600 mx-auto mb-3" />
      <h3 class="font-medium text-zinc-700 dark:text-zinc-300">暂无符合条件的知识资产</h3>
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-1.5 text-xs font-mono">
              <span class="px-1.5 py-0.5 rounded font-bold" :class="item.grade === 'S' || item.grade === 'A' ? 'bg-emerald-100 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300'">
                {{ item.grade }} 级
              </span>
          ...
          <div class="flex flex-wrap items-center gap-1.5 pt-1">
            <span v-for="tag in item.tags" :key="tag" class="text-[10px] px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400">
              #{{ tag }}
            </span>
            <span v-if="item.sourceCwd" class="text-[10px] text-zinc-400 font-mono truncate max-w-[200px]" :title="item.sourceCwd">
    ...
    <UModal v-model:open="isDetailOpen">
      <template #content>
        <div v-if="currentItem" class="p-6 space-y-5 max-h-[85vh] overflow-y-auto">
          ...
          <div class="space-y-4 text-xs">
            <div>
              <h4 class="font-bold text-zinc-700 dark:text-zinc-300 mb-1 flex items-center gap-1">
                <UIcon name="i-lucide-help-circle" class="w-3.5 h-3.5" /> 背景与痛点 (Context)
              </h4>
            ...
            <div>
              <h4 class="font-bold text-zinc-700 dark:text-zinc-300 mb-1 flex items-center gap-1">
                <UIcon name="i-lucide-check-circle" class="w-3.5 h-3.5" /> 核心方案与决策 (Decision)
              </h4>
            ...
            <div>
              <h4 class="font-bold text-zinc-700 dark:text-zinc-300 mb-1 flex items-center gap-1">
                <UIcon name="i-lucide-target" class="w-3.5 h-3.5" /> 落地结果与影响 (Consequences)
              </h4>
          ...
          <div class="flex items-center justify-between pt-4 border-t border-zinc-200 dark:border-zinc-800">
            <span v-if="copySuccess" class="text-xs text-emerald-600 flex items-center gap-1">
