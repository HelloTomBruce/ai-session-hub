...
const handleDistill = async () => {
  ...
  try {
    ...
    toast.add({ title: (err as { message?: string } | null | undefined)?.message || '知识提炼失败', color: 'error', icon: 'i-lucide-alert-triangle' })
  } finally {
    clearInterval(elapsedTimer)
...
const archiveAllAdrs = async () => {
  ...
  try {
    ...
    toast.add({ title: `归档失败: ${(err as { message?: string }).message}`, color: 'error', icon: 'i-lucide-alert-triangle' })
  } finally {
  ...
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-5">
      ...
      <div class="lg:col-span-7">
        ...
        <div
          v-if="isDistilling"
          class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-5 space-y-4 shadow-sm animate-fade-in"
        >
          ...
          <div class="p-3 bg-zinc-50 dark:bg-zinc-800/60 rounded border border-zinc-200/70 dark:border-zinc-700/60 text-xs text-zinc-700 dark:text-zinc-300 flex items-center gap-2">
            ...
            <span
              class="font-medium font-mono flex-1 min-w-0 truncate"
              :title="distillStatus"
            >{{ distillStatus || '正在执行 Map-Reduce 提炼任务...' }}</span>
          ...
          <div
            v-if="mapSessions.length"
            class="rounded border border-zinc-200/70 dark:border-zinc-700/60 overflow-hidden"
          >
            ...
            <div class="divide-y divide-zinc-100 dark:divide-zinc-800/80 max-h-[220px] overflow-y-auto">
              <div
                v-for="m in mapSessions"
                :key="m.id"
                class="px-3 py-1.5 flex items-center gap-2 text-[11px]"
                :class="m.state === 'active' ? 'bg-purple-50/60 dark:bg-purple-950/20' : ''"
              >
                <UIcon
                  :name="mapStateIcon(m.state)"
                  class="w-3.5 h-3.5 shrink-0"
                  :class="mapStateClass(m.state)"
                />
                <span
                  class="flex-1 min-w-0 truncate"
                  :class="m.state === 'pending' ? 'text-zinc-400 dark:text-zinc-500' : 'text-zinc-700 dark:text-zinc-300'"
                  :title="m.title"
                >
                  {{ m.title }}
                </span>
                <span
                  v-if="m.state === 'active'"
                  class="shrink-0 font-mono text-[10px] text-purple-500 dark:text-purple-400"
                >
                  已生成 {{ m.chars }} 字符
                </span>
                <span
                  class="shrink-0 text-[10px]"
                  :class="m.state === 'pending' ? 'text-zinc-300 dark:text-zinc-600' : 'text-zinc-400 dark:text-zinc-500'"
                >
            ...
            <div
              v-if="activeMapText"
              class="border-t border-zinc-200/70 dark:border-zinc-700/60"
            >
              ...
              <div class="p-3 text-zinc-500 dark:text-zinc-400 font-mono text-[10px] max-h-[120px] overflow-y-auto whitespace-pre-wrap leading-relaxed break-all">
                {{ activeMapText }}<span class="inline-block w-1 h-3 bg-purple-500 dark:bg-purple-400 ml-0.5 animate-pulse align-middle" />
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div class="p-4 bg-zinc-50/90 dark:bg-zinc-800/50 text-zinc-700 dark:text-zinc-200 font-mono text-xs rounded-lg border border-zinc-200 dark:border-zinc-700 max-h-[360px] overflow-y-auto whitespace-pre-wrap leading-relaxed shadow-xs">
              {{ streamChunkText }}<span class="inline-block w-1.5 h-3.5 bg-purple-600 dark:bg-purple-400 ml-0.5 animate-pulse align-middle" />
            </div>
          </div>
          <div
            v-else-if="!mapSessions.length"
            class="py-12 flex flex-col items-center justify-center text-zinc-400 space-y-2"
          >
            <UIcon
              name="i-lucide-cpu"
              class="w-8 h-8 opacity-40 animate-pulse"
            />
            <p class="text-xs">
              正在分析会话思维链与工具操作上下文...
            </p>
        ...
        <div
          v-else-if="report"
          class="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-800 p-5 space-y-4 shadow-sm"
        >
          <div class="flex flex-col gap-3 border-b border-zinc-100 dark:border-zinc-800 pb-3">
            <div>
              ...
              <p class="text-xs text-zinc-400 mt-0.5">
                已聚合解析 {{ report.sessionCount }} 个历史会话
              </p>
            </div>
          ...
          >
            ...
            <div class="space-y-1.5">
              ...
              <ul class="space-y-1 text-xs text-zinc-700 dark:text-zinc-300">
                <li
                  v-for="(act, i) in report.actionsDone"
                  :key="i"
            ...
            <div
              v-if="report.technicalDecisions?.length"
              class="space-y-1.5"
            >
              <h3 class="text-xs font-semibold text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5">
                <UIcon
                  ...
                  class="w-3.5 h-3.5 text-zinc-500"
                /> 2. 思考路径与技术决策 (Thinking & Decisions)
              </h3>
              <ul class="space-y-1 text-xs text-zinc-700 dark:text-zinc-300">
                <li
                  v-for="(dec, i) in report.technicalDecisions"
                  :key="i"
                  class="flex items-start gap-1.5"
                >
                  <span class="text-zinc-400">◆</span>
            ...
            <!-- Section: Learnings -->
            <div
              v-if="report.keyLearnings?.length"
              class="space-y-1.5"
            >
              <h3 class="text-xs font-semibold text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5">
                <UIcon
              ...
              <ul class="space-y-1 text-xs text-zinc-700 dark:text-zinc-300">
                <li
                  v-for="(lrn, i) in report.keyLearnings"
                  :key="i"
                  class="flex items-start gap-1.5"
                >
            ...
            <!-- Section: Tools -->
            <div
              v-if="report.toolsAndCommands?.length"
              class="space-y-1.5"
            >
              <h3 class="text-xs font-semibold text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5">
                <UIcon
        ...
        <div
          v-else
          class="h-full min-h-[380px] flex flex-col items-center justify-center bg-white dark:bg-zinc-900 rounded-lg border border-dashed border-zinc-200 dark:border-zinc-800 p-8 text-center text-zinc-400"
        >
          <UIcon
            name="i-lucide-file-text"
            class="w-10 h-10 mb-2 opacity-30"
          />
          <h3 class="font-medium text-xs text-zinc-700 dark:text-zinc-300">
            暂无知识提炼报告
          </h3>
          <p class="text-[11px] text-zinc-400 mt-0.5 max-w-xs">
            在左侧勾选你想要复盘的会话，点击“开始提炼总结”即可生成报告与 ADR 决策集
          </p>
        </div>
      </div>
    </div>
