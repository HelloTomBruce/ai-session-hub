...
const handleSearch = async () => {
  ...
  try {
    ...
    if (err?.statusCode === 503) {
      ...
      error.value = err?.data?.message || err?.message || '搜索请求失败'
    }
    results.value = []
    total.value = 0
  } finally {
...
<template>
  <div class="space-y-5">
    ...
    <!-- Results -->
    <div v-if="hasSearched && !isSearching && !error" class="space-y-3">
      <!-- Result Count -->
      ...
      <div v-if="results.length === 0" class="text-center py-12">
        <UIcon name="i-lucide-file-search-2" class="w-12 h-12 mx-auto text-zinc-300 dark:text-zinc-600 mb-3" />
        <p class="text-sm text-zinc-500 dark:text-zinc-400">没有找到匹配的内容</p>
        <p class="text-xs text-zinc-400 dark:text-zinc-500 mt-1">试试换一个关键词，或先同步缓存</p>
      </div>

      <!-- Result Cards -->
      <div v-for="r in results" :key="r.rowid" class="group">
        <div
        ...
        >
          ...
          <div class="px-3.5 py-2 flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800/80">
            <div class="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
              <UIcon :name="sourceMeta[r.platform]?.icon || 'i-lucide-terminal'" class="w-3.5 h-3.5" />
    ...
    <div v-if="!hasSearched && !isSearching" class="text-center py-16">
      <UIcon name="i-lucide-search" class="w-16 h-16 mx-auto text-zinc-200 dark:text-zinc-700 mb-4" />
      <h3 class="text-sm font-medium text-zinc-500 dark:text-zinc-400">搜索 AI 编码会话</h3>
