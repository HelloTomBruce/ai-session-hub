...
export function distillSessionsContent(sessions: Array<{ session: UnifiedSession, messages: SessionMessage[] }>): DistillReport {
  ...
  for (const item of sessions) {
    ...
    for (const msg of item.messages) {
      ...
      if (msg.thought) {
        ...
        for (const tl of thoughtLines.slice(0, 3)) {
          if (/方案|选择|决定|原因|需要|建议|由于|采用/i.test(tl)) {
            technicalDecisions.push(tl)
          } else if (/注意|避免|报错|bug|fix|解决|修复|gotcha/i.test(tl)) {
  ...
  const uniqueLearnings = Array.from(new Set(keyLearnings)).slice(0, 15)
  const uniqueDecisions = Array.from(new Set(technicalDecisions)).slice(0, 15)
  const uniqueTools = Array.from(new Set(toolsAndCommands)).slice(0, 15)
  const uniqueTodos = Array.from(new Set(todos)).slice(0, 10)
...
## 💡 2. 关键技术决策与选型 (Technical Decisions)
